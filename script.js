...
function takeInput(previousInput) { // Function for taking user input. 
  if (previousInput) {
      return(prompt("It appears your password either contained letters or was outside the range of 8 to 128. Please input a valid length."));
  } else {
      return(prompt("Please enter a password length between 8 and 128. (Note: Use numbers, not letters)"));
  }
};

function generatePassword() {
  let passwordLength = takeInput(false);
    while (parseInt(passwordLength) < 8 || parseInt(passwordLength) > 128 || isNaN(passwordLength)) { // Critera for valid input; must be a number and between 8 and 128, inclusive.
      passwordLength = takeInput(true);
    ...
    passwordLength = parseInt(passwordLength); // Change input from string to integer for later computations.

    if (passwordLength !== null) { // Stops following code if user chooses to press cancel for initial prompt.
